...
for num in bingoCall:
    ...
    for bingoSheet in bingoData:
        ...
        if (bingoSheet[0] == marked or 
            ...
            bingoCol3 == marked or
            bingoCol4 == marked ):
            # Keep Track of all sheets won until there is one sheet left
            sheetsWon += 1
            if sheetsWon == totalSheets:
                lastWinningSheet = bingoSheet
                winningNumber = int(num)
                break
        else:
            # Remove all winning sheets, keep only the sheets that have yet to win
            updatedBingoData.append(bingoSheet)
    if winningNumber is not None:
        break
    bingoData = updatedBingoData

# calculate the score for the sheet that won last
sum = 0
for bingoRow in lastWinningSheet:
